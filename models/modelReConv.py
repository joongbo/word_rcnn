...
from ReConvLayer import ReConvLayer
from FullConnLayerDrop import FullConnLayerDrop
from ReConvLayerDrop import ReConvLayerDrop
...
def building_model(opts):
    ...
    for l in xrange(l_CuConv_layer):
        ...
        if l+1 == l_CuConv_layer:
            for p in xrange(p_CuConv_layer):
                if opts['dropRC']:
                    layerCuConv = ReConvLayerDrop(rng, trng, inputs[p], input_shapes[p], fltrCuConv[l][p], opts['numStep'], 
                    ...
                    layerCuConv = ReConvLayer(rng, trng, inputs[p], input_shapes[p], fltrCuConv[l][p], opts['numStep'], 
                                      LRN=opts['LRN'], BN=opts['BN'], BN_mode=0,
                                      pool=True, pool_mode=opts['poolMode'], 
                                      L=l_CuConv_layer, l=l+1, k_top=opts['kTop'], s=s,
